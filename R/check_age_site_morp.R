...
data <- list(age = age,
 ...
 un_age <- list(0:2, 10:14, 6:14, 9:14, 0:8, 6:14, 0:8, 0:5,
                0:5, 0:3, 8:14, 0:14, 0:5, 0:5, 0:4, 0:4)
 un_sub <- list(21, 41, 50, 61, 62, 71, 72, 81,
                82, 83, 102, 104, 112, 113, 115, 116)
 age_sub <- function(x){
 ...
 result[[20]] <- age >= 15 & age < 20 & topo %in% gen_topo(170:179) & morp < 9590
 result[[21]] <- age >= 15 & age < 20 & topo %in% gen_topo(c(180:189,330:349)) &
   (!substr(morp,1,3) == "824")
 result[[22]] <- age > 45 & topo %in% gen_topo(580:589) & morp == 9100
 result[[23]] <- age >= 15 & age <= 25 & morp %in% c(9732, 9823)
 result[[24]] <- age >= 15 & (morp %in% c(8910, 8960, 8970, 8981, 8991, 9072, 9470,
                                   9687) | substr(morp, 1, 3) == "951")
