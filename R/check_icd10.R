...
check_icd10 <- function(icd10) {
  format_check <- valid_icd10(toupper(icd10))
  init_letter <- substring(icd10, 1, 1)
  icd_number <- as.numeric(gsub("[^0-9\\.]", "", icd10))
  ress <- ifelse(
    toupper(init_letter) == "D",
    generate_icd10(icd_number * 10, "D"),
    ifelse(toupper(init_letter) == "C", generate_icd10(icd_number * 10), NA)
  )
  check <- ress == icd10
...
valid_icd10 <- function(icd10) {
  CC <- c(1, 7, 12, 19, 20, 23, 33, 37, 52, 55, 56, 58, 61, 64, 65, 66, 73, 97)
  DD <- c(24, 27, 34, 45)
  special_icd10 <- c(sprintf("C%02d", CC), paste0("D", DD))
  pattern <- "^[CD]\\d{2}\\.\\d$"
...
generate_icd10 <- function(x, init = "C", na.drop = FALSE) {
  if (init == "C") {
    x <- ifelse(x %in% not_icd10$C | x > 979, NA, x)
    if(na.drop){x <- x[!is.na(x)]}
    res <- ifelse(x %in% (
      c(1, 7, 12, 19, 20, 23, 33, 37, 52, 55, 56, 58, 61, 64, 65, 66, 73, 97) * 10
    ),
