...
#'
#' @param grade grade code
#' @param morp morphology code
#'
...
check_grade_morp <- function(grade, morp) {
  ...
  check4 <- (!grade == 5) &
    morp %in% c(9702, 9705, 9706, 9708, 9709, 9717, 9718, 9729, 9827,
                9834, 9837)
  ...
  check12 <- (!grade == 4) &
    morp %in% c(8020, 8021, 8805, 9062, 9082, 9390, 9392, 9401, 9451,
                9505, 9512)
  check <- check2 | check3 | check4 | check5 | check6 | check7 |
    check8 | check9 | check10 | check11 | check12
  check <- !check
  type <- ifelse(check, 1, 2)
  message <- ifelse(check2, 212, ifelse(check3, 213, ifelse(check4, 214, ifelse(
    check5, 215, ifelse(check6, 216, ifelse(check7, 217, ifelse(
      check8, 218, ifelse(check9, 219, ifelse(
        check10, 220, ifelse(check11, 221, ifelse(check12, 222, 0))
      ))
    )))
  ))))
  res <- list()
